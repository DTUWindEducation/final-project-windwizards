...
@pytest.fixture
def prepared_blade_elements(
    sample_blade_elements, sample_operational_condition, sample_airfoil
):
...
def sample_blade(prepared_blade_elements, sample_operational_characteristics):
    ...
    blade = Blade(
        elements=prepared_blade_elements,
...
def test_compute_aerodynamic_performance(
    sample_blade_element_theory, sample_operational_condition
):
    """Test the compute_aerodynamic_performance method."""
    # Call the method being tested
    total_thrust, total_torque, total_power, ct, cp = (
        sample_blade_element_theory.compute_aerodynamic_performance(
    ...
    assert (
        total_torque >= 0
    ), "Torque should be non-negative for a properly oriented wind turbine"
    assert total_power >= 0, "Power should be non-negative"
    assert 0 <= ct <= 1.0, f"Thrust coefficient (CT={ct}) should be between 0 and 1"
    assert (
        0 <= cp <= 0.593
    ), f"Power coefficient (CP={cp}) should be between 0 and Betz limit (0.593)"

    # Verify that power = torque * omega
    assert (
        abs(total_power - total_torque * sample_operational_condition.omega) < 1e-10
    ), "Power should equal torque times angular velocity"


def test_element_forces_calculation(
    sample_blade_element_theory, sample_operational_condition
):
    ...
    for element in sample_blade_element_theory.blade.elements:
        ...
        assert (
            element.V_rel is not None
        ), "Element relative velocity should be calculated"

        # Verify relations between forces
        assert (
            abs(
                element.Fn
                - (element.L * np.cos(element.phi) + element.D * np.sin(element.phi))
            )
            < 1e-10
        ), "Normal force calculation error"
        assert (
            abs(
                element.Ft
                - (element.L * np.sin(element.phi) - element.D * np.cos(element.phi))
            )
            < 1e-10
        ), "Tangential force calculation error"
...
def test_extreme_conditions(sample_blade_element_theory):
    ...
    normal_wind_condition = OperationalCondition(
        wind_speed=10.0, rho=1.225, num_blades=3
    )
    ...
    assert (
        results_high_wind[0] > results_normal_wind[0]
    ), "Thrust should increase with wind speed"
    assert (
        results_high_wind[2] > results_normal_wind[2]
    ), "Power should increase with wind speed"
...
def test_integration_with_blade_class(sample_blade, sample_operational_condition):
    ...
    # Perform calculation
    total_thrust, total_torque, total_power, ct, cp = (
        bet.compute_aerodynamic_performance(
            operational_condition=sample_operational_condition
        )
    )
...
def test_init_method():
    ...
    assert (
        bet.blade is mock_blade
    ), "The 'blade' attribute should reference the provided blade"
